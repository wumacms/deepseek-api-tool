...
<template>
  <div class="flex flex-col gap-2 items-end">
    ...
    <div
      v-if="msg.role === 'user'"
      class="max-w-[85%] sm:max-w-[70%] bg-[var(--user-bubble-bg)] border border-[var(--user-bubble-border)] px-4 py-2.5 rounded-none text-sm text-[var(--user-bubble-text)] whitespace-pre-wrap shadow-sm leading-relaxed"
    >
      {{ msg.content }}
    </div>
    ...
    <div v-else-if="msg.role === 'assistant'" class="w-full flex flex-col gap-3">
      ...
      >
        ...
        <div
          class="text-xs text-thinking-foreground/80 font-mono whitespace-pre-wrap leading-relaxed text-left markdown-content"
          v-html="renderMarkdownCached(msg.reasoning_content)"
        ></div>
      </div>

      <!-- 回答正文 -->
      <div class="bg-[var(--assistant-bg)] border border-[var(--assistant-border)] px-5 py-4 rounded-none text-sm text-[var(--assistant-text)] leading-relaxed shadow-sm">
        <!-- 中断标记 -->
        <div v-if="msg.interrupted" class="text-[10px] text-warning/80 mb-2 flex items-center gap-1 select-none">
        ...
        <!-- 底部操作栏 -->
        <div class="flex items-center justify-between mt-3 pt-2.5 border-t border-border/20 text-xs text-muted-foreground select-none">
          <div class="flex items-center gap-4">
            <button
              ...
              title="复制全文"
            >
              <Check v-if="copiedIndices[index]" class="w-3.5 h-3.5 text-success" />
          ...
          <div v-if="msg.usage" class="text-[11px] text-muted-foreground/60 font-mono flex items-center gap-2">
            <span
              ...
              class="cursor-help hover:text-muted-foreground transition-colors"
            >
              Tokens: <span class="text-muted-foreground font-bold">{{ msg.usage.total_tokens }}</span>
            </span>
            <span class="w-1.5 h-1.5 bg-border rounded-full"></span>
            <span>
              花费: <span class="text-primary font-bold">¥{{ formatCost(calculateCost(msg.model || currentModel, msg.usage)) }}</span>
            </span>
          </div>
        </div>
      </div>
